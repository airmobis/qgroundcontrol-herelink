...
void GeoWork::loadSettings() {
    ...
    const TokenStatus st { static_cast<TokenStatus>(m_settings.value(QStringLiteral("tokenStatus"), 0).toUInt()) };
    ...
    if (any) {
        qInfo() << "[GeoWork] Settings loaded. deviceName=" << m_deviceName
                << " tokenLen=" << m_bearerToken.size() << " tokenStatus=" << static_cast<std::uint32_t>(m_tokenStatus);
    }
}
