...
Item {
    ...
    Component.onCompleted: {
        try {
            console.log("[DBG] GeoWork object:", GeoWork)
            console.log("[DBG] tokenStatus:", GeoWork.tokenStatus, "deviceName:", GeoWork.deviceName)
        } catch (e) {
            console.warn("[DBG] GeoWork access failed:", e)
        }
        try {
            var v = QGroundControl.multiVehicleManager.activeVehicle
            console.log("[DBG] activeVehicle exists?", !!v)
            if (v && v.gps && v.gps.count) console.log("[DBG] sats:", v.gps.count.rawValue)
        } catch (e2) {
            console.warn("[DBG] vehicle access failed:", e2)
        }
    }
    ...
    Loader {
        ...
        onStatusChanged: {
             console.log("[Loader] status:", status, "error:", errorString())
            if (status === Loader.Ready && item) {
                item.anchors.fill = _root
                item.visible = false
                console.log("[GeoWork] Settings panel loaded")
            } else if (status === Loader.Error) {
                console.warn("[GeoWork] Settings panel load ERROR:", errorString())
            }
        }
    }
                    ...
                    Image {
                        ...
                        source: (function(){
                            try {
                                ...
                                console.warn("[DBG] settings icon binding error:", e)
